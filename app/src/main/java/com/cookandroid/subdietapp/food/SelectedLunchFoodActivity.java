...
public class SelectedLunchFoodActivity extends AppCompatActivity {
    ...
    private void showDialog() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(SelectedLunchFoodActivity.this);
    ...
    private boolean checkPermission() {
        int result = ContextCompat.checkSelfPermission(SelectedLunchFoodActivity.this,
    ...
    private void requestPermission() {
        if(ActivityCompat.shouldShowRequestPermissionRationale(SelectedLunchFoodActivity.this,
                Manifest.permission.WRITE_EXTERNAL_STORAGE)){
            Toast.makeText(SelectedLunchFoodActivity.this, "권한 수락이 필요합니다.",
                    Toast.LENGTH_SHORT).show();
        }else{
            ActivityCompat.requestPermissions(SelectedLunchFoodActivity.this,
    ...
    private void camera() {
        int permissionCheck = ContextCompat.checkSelfPermission(
                SelectedLunchFoodActivity.this, android.Manifest.permission.CAMERA);

        if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(SelectedLunchFoodActivity.this,
                    new String[]{Manifest.permission.CAMERA},
                    1000);
            Toast.makeText(SelectedLunchFoodActivity.this, "카메라 권한이 필요합니다.",
                    Toast.LENGTH_SHORT).show();
            return;
        } else {
            Intent i = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            if (i.resolveActivity(SelectedLunchFoodActivity.this.getPackageManager()) != null) {

                // 사진의 파일명을 만들기
                String fileName = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
                photoFile = getPhotoFile(fileName);

                Uri fileProvider = FileProvider.getUriForFile(SelectedLunchFoodActivity.this,
                        "com.cookandroid.subdietapp.fileprovider", photoFile);
                i.putExtra(MediaStore.EXTRA_OUTPUT, fileProvider);
                startActivityForResult(i, 100);

            } else {
                Toast.makeText(SelectedLunchFoodActivity.this, "카메라 앱이 필요합니다.",
                        Toast.LENGTH_SHORT).show();
            }
        }
...
//                imageView.setImageBitmap( getBitmapAlbum( imageView, albumUri ) );
    ...
    private void visionNetwokData() {
        ...
        Retrofit retrofit = VisionNetworkClient.getRetrofitClient(SelectedLunchFoodActivity.this);
